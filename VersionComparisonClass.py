...
class CheckVersion():
  def __init__(self, VersionRangeList, VersionToCheck):
    ...
    self.VersionRangeInput = [interval.lower().strip() for versionRange in VersionRangeList for interval in versionRange.split(',')]
  ...
  def VersionRangePrefixHelper(self, versionPoint):
    ...
    '''
    VersionNumber = re.search(self.regexVersionPattern, versionPoint).group(1) #Only select the first version group in case the suffix has another 1.2..
  ...
  def FixLonelyBracket(self, versionPoint):
    ...
    if len(versionPoint) == 1: # type of range: [,XXX] - gets only bracket : '[' 
      if versionPoint[0] == '[' or versionPoint[0] == '(':
        ...
        versionPoint = '10000000' + versionPoint[0] #Arbitrarily huge number to satisfy open ended upper limit.
    return versionPoint
